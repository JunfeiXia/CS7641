...
def plot_nn_performances(x_train, y_train, random_seeds, **kwargs):
    ...
    for i, algorithm in enumerate(algorithms):
        ...
        otherfunctions.plot_helper(x_axis=kwargs[max_iters[i]], y_axis=np.array(train_curves), label='{} train'.format(acronyms[i]))
        otherfunctions.plot_helper(x_axis=kwargs[max_iters[i]], y_axis=np.array(val_curves), label='{} val'.format(acronyms[i]))

        # Plot training time figure for current algorithm
        plt.figure(train_times_figure)
        otherfunctions.plot_helper(x_axis=kwargs[max_iters[i]], y_axis=np.array(train_time_curves), label=acronyms[i])

    # Set title and labels to training and validation figure
    plt.figure(train_val_figure)
    otherfunctions.set_plot_title_labels(title='Neural Network - Loss vs. iterations',
                                x_label='Iterations',
                                y_label='Loss')
    ...
    plt.figure(train_times_figure)
    otherfunctions.set_plot_title_labels(title='Neural Network - Time vs. iterations',
                                x_label='Iterations',
