...
def plot_ga_mimic_optimization(problem, param_name, random_seeds, **kwargs):
    ...
    plt.figure()
    otherfunctions.plot_helper(x_axis=kwargs[param_name + 's'], y_axis=np.array(ga_curve).T, label='GA')
    otherfunctions.plot_helper(x_axis=kwargs[param_name + 's'], y_axis=np.array(mimic_curve).T, label='MIMIC')
    otherfunctions.set_plot_title_labels(title='{} - Objective vs. {}'.format(kwargs['plot_name'], label),
                                x_label=label,
...
def plot_sa_optimization(problem, random_seeds, **kwargs):
    ...
    # Plot, set title and labels
    plt.figure()
    otherfunctions.plot_helper(x_axis=kwargs['sa_decay_rates'], y_axis=np.array(sa_curve).T, label='SA')
    otherfunctions.set_plot_title_labels(title='{} - Objective vs. temperature exponential decay rate'.format(kwargs['plot_name']),
...
def plot_performances(problem, random_seeds, **kwargs):
    ...
    for random_seed in random_seeds:
        ...
        _, best_objective, objective_curve = random_hill_climb(problem,
                                                               ...
                                                               state_fitness_callback=otherfunctions.time_callback,
                                                               callback_user_info=[])

        rhc_objectives.append(objective_curve[:,1])
        # rhc_objectives.append(best_objective)
        rhc_times.append(otherfunctions.times)
        ...
        sa_objectives.append(objective_curve[:,1])
        # sa_objectives.append(best_objective)
        sa_times.append(otherfunctions.times)
        ...
        ga_objectives.append(objective_curve[:,1])
        # ga_objectives.append(best_objective)
        ga_times.append(otherfunctions.times)
        print('GA: best_objective = {:.3f}'.format(best_objective))
        ...
        mimic_objectives.append(objective_curve[:,1])
        # mimic_objectives.append(best_objective)
        mimic_times.append(otherfunctions.times)
        print('MIMIC: best_objective = {:.3f}'.format(best_objective))
    ...
    plt.figure()
    otherfunctions.plot_helper(x_axis=rhc_iterations, y_axis=np.array(rhc_objectives), label='RHC')
    otherfunctions.plot_helper(x_axis=sa_iterations, y_axis=np.array(sa_objectives), label='SA')
    otherfunctions.plot_helper(x_axis=ga_iterations, y_axis=np.array(ga_objectives), label='GA')
    otherfunctions.plot_helper(x_axis=mimic_iterations, y_axis=np.array(mimic_objectives), label='MIMIC')
    otherfunctions.set_plot_title_labels(title='{} - Objective versus iterations'.format(kwargs['plot_name']),
                                x_label='Iterations',
                                y_label=kwargs['plot_ylabel'])
    ...
    plt.figure()
    otherfunctions.plot_helper(x_axis=rhc_iterations, y_axis=np.array(rhc_times), label='RHC')
    otherfunctions.plot_helper(x_axis=sa_iterations, y_axis=np.array(sa_times), label='SA')
    otherfunctions.plot_helper(x_axis=ga_iterations, y_axis=np.array(ga_times), label='GA')
    otherfunctions.plot_helper(x_axis=mimic_iterations, y_axis=np.array(mimic_times), label='MIMIC')
    otherfunctions.set_plot_title_labels(title='{} - Time versus iterations'.format(kwargs['plot_name']),
                                x_label='Iterations',
